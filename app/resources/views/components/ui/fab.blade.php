...
<a href="{{ $href }}" wire:navigate aria-label="{{ $label }}" title="{{ $label }}"
    {{ $attributes->class('fixed bottom-[calc(max(0.75rem,env(safe-area-inset-bottom))+4.75rem)] right-4 z-30 inline-flex h-14 min-w-14 items-center justify-center gap-2 rounded-full bg-accent px-4 font-[family-name:var(--font-display)] text-xl font-semibold text-on-accent shadow-[0_10px_30px_-8px_rgb(var(--c-shadow)/0.45)] transition hover:brightness-110 active:scale-95 lg:bottom-8 lg:right-8') }}>
