...
<x-ui.field :label="$label" :for="$id" :hint="$hint" :name="$name" :required="$required" :class="$attributes->get('class')">
    <div x-data="markdownEditor({ property: @js($property) })"
        ...
        <textarea x-ref="code" x-show="mode === 'code'" x-cloak id="{{ $id }}" rows="{{ $rows }}" placeholder="{{ $placeholder }}"
        ...
        <div x-ref="preview" x-show="mode === 'preview'" contenteditable="true" x-html="html"
